...
class LSTM_MLP(Module):
    def __init__(self, state_dim, action_dim, lstm_hidden_dim, num_layers, mlp_hidden_dims, output_dim, IsDemo=True):
...
#         # self.conv1.node_dim = -1
#         # self.conv2.node_dim = -1
#
#     def forward(self, x):
#         data = Data(x=x[0], edge_index=self.edge_index)
...
# #         self.edge_index = torch.tensor(edge_index, dtype=torch.long).t().to(DEVICE)
# #
# #     def forward(self, x):
# #         data = Data(x[0], self.edge_index)
...
class Critic(Module):
    def __init__(self, state_dim, action_dim, n_quantiles, n_nets, IsDemo=True):
        super().__init__()
        self.nets = []
...
class Actor(Module):
    def __init__(self, state_dim, action_dim, IsDemo=True):
        super().__init__()
